...
unsigned int shellRunCommand(Shell *shell, ShellCommand *command)
{
    ...
    if (command->attr.attrs.type == SHELL_TYPE_CMD_MAIN)
    {
        ...
        int (*main_func)(int, char **) = (int (*)(int, char **))command->data.cmd.function;
        returnValue = main_func(shell->parser.paramCount,
                                 shell->parser.param);
...
int shellExecute(int argc, char *argv[])
{
    ...
    if (shell && argc >= 2)
    {
        void *func = (void *)shellExtParsePara(shell, argv[1]);
